...
# Bucket root must be /data, not /. fileblob rejects keys under a "/" root
# (HasPrefix check against "//"), which surfaces as "escapes bucket root".
...
RUN apk --no-cache add ca-certificates wget mosquitto-clients font-noto-cjk && \
    if [ "$TARGETARCH" != "arm" ] || [ "$TARGETARCH" != "riscv64" ]; then apk --no-cache add libwebp libavif libheif libjxl; fi
